...
class block_afs_external extends external_api {
    ...
    public static function request_files($draw, $start, $length, $search, $cols, $order) {
        ...
        $dataSQL = "SELECT `" . implode("`, `", self::pluck($columns, 'db')) . "` FROM {block_afs}";

        $search = null;

        $resFilterLengthSQL = "SELECT COUNT({$primaryKey}) FROM {block_afs}";
        ...
        $resTotalLengthSQL = "SELECT COUNT({$primaryKey}) FROM {block_afs}";
