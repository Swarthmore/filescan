...
/**
 * @package   block_afs_
 * @copyright 2020 Swarthmore College ITS
...
if (!$course = $DB->get_record('course', array('id' => $courseid))) {
  print_error('invalidcourse', 'block_afs', $courseid);
}

require_login($course);

$PAGE->set_url('/block/afs/course.php', array('id' => $courseid));
$PAGE->set_pagelayout('standard');
$PAGE->set_heading(get_string('viewdetailspage', 'block_afs'));
...
if (!$canview) {
  echo get_string('cannot_view', 'block_afs');
  return;
}
...
{
  // Params.
  $header = get_string("table:{$id}_header", 'block_afs');
  $link = get_config('block_afs', $id . '_help');
  $title = get_string('helptitle', 'block_afs');
...
foreach ($file_list as $f) {
  if ($f['sectionNumber'] != $previous_section_number) {
    ...
    $output_html .= "<tr><th style=''>" . get_string('table:mod_header', 'block_afs') . '</th>';
    $output_html .= '<th>' . get_string('table:filename_header', 'block_afs') . '</th>';
    $output_html .= '<th>' . get_string('table:status_header', 'block_afs') . '</th>';
  ...
  $output_html .= "<tr><td style='text-align:center'>" . $mod_link . "<td><a href=\"" . $f['fileurl'] . "\" target='_blank'>" . $f['filename'] . "</a></td>
	<td style='text-align:center;'><span title=\"" . get_string('table:status_header', 'block_afs') . '">' . $overallstatus . "</span></td>
	<td style='text-align:center;border-left-width:2px;'><span title=\"" . get_string('table:text_check_header', 'block_afs') . '">' . $hastext . "</span></td>
	<td style='text-align:center'><span title=\"" . get_string('table:title_check_header', 'block_afs') . '">' . $hastitle . "</span></td>
	<td style='text-align:center'><span title=\"" . get_string('table:lang_check_header', 'block_afs') . '">' . $haslanguage . "</span></td>
	<td style='text-align:center'><span title=\"" . get_string('table:outline_check_header', 'block_afs') . '">' . $hasoutline . "</span></td>
	</tr>";
...
if ($ocr_summary['Accessible'] > 0) {
  $summary_text .= '<BR>' . $success_icon . get_string('summary:files_accessible', 'block_afs', $ocr_summary['Accessible']);
}

if ($ocr_summary['Partially Accessible'] > 0) {
  $summary_text .= '<BR>' . $partial_success_icon . get_string('summary:files_partially_accessible', 'block_afs', $ocr_summary['Partially Accessible']);
}

if ($ocr_summary['Inaccessible'] > 0) {
  $summary_text .= '<BR>' . $fail_icon . get_string('summary:files_inaccessible', 'block_afs', $ocr_summary['Inaccessible']);
}

if ($ocr_summary['Unknown'] > 0) {
  $summary_text .= '<BR>' . $unknown_icon . get_string('summary:files_accessibility_unknown', 'block_afs', $ocr_summary['Unknown']);
}


// Prepend summary to top of content
$output_html = "<h1>" . get_string('summary:title', 'block_afs') . "</h1>" . $summary_text . "<BR><BR>" . $output_html;
...
function create_file_item($DB, $cm, $cmtype, $mod_name, $sectionName, $section_no, $file)
{
  ...
  $results = $DB->get_record("block_afs", array('contenthash' => $file->get_contenthash()), $fields = '*');
