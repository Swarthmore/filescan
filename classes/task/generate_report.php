...
class generate_report extends \core\task\scheduled_task
{
    public function get_name()
    {
        ...
        return get_string('task:generate_report', 'block_afs');
    }
    ...
    public function execute()
    {
        ...
        $DB->execute("update {block_afs} set courseinfo=NULL");
        ...
        $query = '
          ...
          inner join {block_afs} fs on (f.contenthash = fs.contenthash)
        ...
        foreach ($pdf_rs as $key => $file) {
            ...
            $courseinfo_results = $DB->get_record('block_afs', array('id' => $file->filescanid), 'courseinfo');
            ...
            $update_filescan = $DB->update_record('block_afs', array('id' => $file->filescanid, 'courseinfo' => json_encode($courseinfo)), false);
            $index++;
